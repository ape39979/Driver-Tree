...
def create_water_driver_tree():
    # Create Digraph
    # We will generate both PNG and SVG. SVG is importable by Visio.
    dot = Digraph("WaterInfrastructureDriverTree")
    ...
    # Critical: fill:#E74C3C, text:white (Red)
    ...
    # D1_A Levers
    # L1 renamed based on user feedback/error snippet implying "Utility Coordination (PWD)" and critical status
    dot.node("L1", "Utility Coordination\n(PWD)", fillcolor=c_critical, fontcolor=t_white)
    ...
    # Render SVG (for Visio)
    dot.render(output_filename, format="svg", cleanup=False)
